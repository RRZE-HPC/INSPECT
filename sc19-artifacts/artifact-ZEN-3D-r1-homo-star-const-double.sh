#! /bin/bash -l
#PBS -l nodes=naples1:ppn=96,walltime=24:0:0
#PBS -N INSPECT_ZEN_bench
...
# in order to be able to generate performance models on AMD Zen at the moment,
# Kerncrafts feature branch 'feature/osaca' has to be used
KERNCRAFT_BINARY=~/kerncraft-osaca/install/bin/kerncraft
...
# select the desired compiler: icc, gcc, clang
CC=icc

# machine files
MACHINE_FILE=${INSPECT_DIR}/machine_files/Zen_EPYC-7451.yml
...
for fDATATYPE in ${DATATYPE}; do
	...
	COMPILER=$(cat ${MACHINE_FILE} | grep ${CC} | sed "s/.*${CC}: /${CC} /")
	...
	echo {\$,$}DATE >> args.txt
	echo {\$,$}COMPILER_VERSION >> args.txt
	...
	${KERNCRAFT_BINARY} -p LC -m ${MACHINE_FILE} ./stencil.c -D N 100 -D M 100 -D P 100 -vvv --cores 1 --compiler ${CC} --ignore-warnings > data/LC.txt
	...
	if [[ ${DO_GRID_SCALING} == 1 ]]; then
		...
		for (( size=10; size<=${LC_3D_L3_N}+10; size=size+10)); do
			...
			KERNCRAFT_ARGS="-m ${MACHINE_FILE} ./stencil.c -D N ${size} -D M ${size} -D P ${size} -vvv --cores 1 --compiler ${CC} --ignore-warnings"
			...
			${KERNCRAFT_BINARY} -p Benchmark -P LC ${KERNCRAFT_ARGS} --no-phenoecm > data/singlecore/Bench_${size}.txt
		done
	...
	if [[ ${DO_THREAD_SCALING} == 1 ]]; then
		...
		for (( threads = 1; threads <= ${cores}; threads++ )); do
			...
			KERNCRAFT_ARGS="-m ${MACHINE_FILE} ./stencil.c -D N ${LC_3D_L3_N} -D M ${LC_3D_L3_N} -D P ${LC_3D_L3_N} -vvv --cores ${threads} --compiler ${CC} --ignore-warnings"
			for pmodel in RooflineIACA ECM Benchmark; do
				${KERNCRAFT_BINARY} -p ${pmodel} -P LC ${KERNCRAFT_ARGS} --no-phenoecm >> data/scaling/${pmodel}_${LC_3D_L3_N}_${threads}.txt
			done
		done
	...
	if [[ ${DO_SPACIAL_BLOCKING} == 1 ]]; then
		...
		for blocking_case in L2 L3; do
			...
			for (( size=10; size<=${LC_3D_L3_N}+10; size=size+10)); do
				...
				likwid-perfctr -f -o data/blocking/${blocking_case}_3D/likwid_${size}.txt -g ENERGY -C S0:0 -m ./stencil_blocking ${args} >> data/blocking/${blocking_case}_3D/likwid_${size}_out.txt
			done
			echo
		done
	fi
	...
	sh ${INSPECT_DIR}/scripts/postprocess_osaca.sh ${INSPECT_DIR}/stencils
